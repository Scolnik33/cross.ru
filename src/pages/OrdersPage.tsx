...
import { fetchOrders, selectOrders } from "../redux/slices/order";
...
const OrdersPage: React.FC = () => {
  ...
  useEffect(() => {
    if (dataAuth?._id) {
      dispatch(fetchOrders(dataAuth._id));
    }
  }, []);
        ...
        {itemsOrders.length > 0 ? (
              ...
              >
                <div className="p-6 border-b border-dark-border">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    {/* <div className="flex items-center gap-4"> */}
                    {/* <span className="text-white font-medium">{order.payment.toLocaleString('ru-RU')} ₽</span> */}
                    {/* <Link
                        to={`/orders/${order._id}`}
          ...
          <div className="bg-dark-card rounded-lg p-12 text-center">
            ...
            <p className="text-light-muted mb-8">
              Вы ещё не сделали ни одного заказа. Начните покупки, чтобы увидеть
              свои заказы здесь.
            </p>
            <Link
              to="/products"
